...
parser.add_argument(
    "--diffusion_steps", default=20, type=int, help="Total DDIM sampling steps"
)
parser.add_argument(
    "--start_step", default=15, type=int, help="Which DDIM step to start the attack"
)
parser.add_argument(
    "--iterations", default=30, type=int, help="Iterations of optimizing the adv_image"
)
parser.add_argument(
    "--res", default=224, type=int, help="Input image resized resolution"
)
parser.add_argument(
    "--model_name",
    default="inception",
...
parser.add_argument(
    "--guidance", default=2.5, type=float, help="guidance scale of diffusion models"
)
parser.add_argument(
    "--attack_loss_weight", default=10, type=int, help="attack loss weight factor"
)
parser.add_argument(
    "--cross_attn_loss_weight",
    default=10000,
    type=int,
    help="cross attention loss weight factor",
)
parser.add_argument(
    "--self_attn_loss_weight",
    default=100,
    type=int,
...
if __name__ == "__main__":
    ...
    model_name = (
        args.model_name
    )  # The surrogate model from which the adversarial examples are crafted.
    ...
    with open(label_path, "r") as f:
        ...
        for i in f.readlines():
            label.append(
                int(i.rstrip()) - 1
            )  # The label number of the imagenet-compatible dataset starts from 1.
    ...
    for ind, image_path in enumerate(all_images):
        tmp_image = Image.open(image_path).convert("RGB")
        tmp_image.save(
            os.path.join(save_dir, str(ind).rjust(4, "0") + "_originImage.png")
        )
