...
def model_transfer(
    ...
    for name in models_transfer_name:
        ...
        f_model = PyTorchClassifier(
            ...
            preprocessing=(
                ...
                if "adv" in name
                else (np.array([0.485, 0.456, 0.406]), np.array([0.229, 0.224, 0.225]))
            ),
            device_type="gpu",
        )
        ...
        accuracy = (
            np.sum((np.argmax(clean_pred, axis=1) - 1) == label) / len(label)
            if "adv" in name
            else np.sum(np.argmax(clean_pred, axis=1) == label) / len(label)
        )
    ...
    table = wandb.Table(
        columns=["model", "clean_accuracy", "adv_accuracy"],
        data=[
            [name, all_clean_accuracy[index], all_adv_accuracy[index]]
            for index, name in enumerate(models_transfer_name)
        ],
    )
