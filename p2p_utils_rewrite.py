...
class AttentionControlEdit(AttentionStore, abc.ABC):
    ...
    def __init__(
        ...
        if isinstance(self_replace_steps, float):
            self_replace_steps_tupe = 0, self_replace_steps
        self.num_self_replace = int(num_steps * self_replace_steps_tupe[0]), int(
            num_steps * self_replace_steps_tupe[1]
        )
...
def get_equalizer(
    ...
    # Ensure values is a list or tuple of floats
    if isinstance(values, torch.Tensor):
        values = values.tolist()
    for word in (
        word_select if isinstance(word_select, (list, tuple)) else [word_select]
    ):
        inds = get_word_inds(text, word, tokenizer)

        for i, v in enumerate(values):
            equalizer[i, inds] = float(v)
    return equalizer


# todo 这里可能有问题，如果不对的话，用被注释的部分换上面的函数
...
def get_word_inds(text: str, word_place: int, tokenizer):
    ...
    if isinstance(word_place, str):
        word_place_list = [i for i, word in enumerate(split_text) if word_place == word]
    elif isinstance(word_place, int):
        word_place_list = [word_place]
    elif isinstance(word_place, list):
        word_place_list = word_place
    else:
        word_place_list = []
    out = []
    if len(word_place_list) > 0:
        words_encode = [
...
def update_alpha_time_word(
    ...
    if type(bounds) is float:
        bounds_tuple = 0, bounds
    start, end = int(bounds_tuple[0] * alpha.shape[0]), int(
        bounds_tuple[1] * alpha.shape[0]
    )
